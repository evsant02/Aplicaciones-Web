...
class dirigirActividad extends formBase
{
    private $actividad;

    public function __construct()
    {
        parent::__construct('dirigirActividad');
    ...
    protected function CreateFields($datos){
        ...
        if (!$this->actividad) {
            return "<p>Actividad no disponible.</p>";
        }

        $voluntario = $this->actividad->getVoluntario() ? $this->actividad->getVoluntario() : "Ningún voluntario asignado";
        ...
        $html = <<<EOF
            ...
            <div class="actividad-detalles">
                ...
                <p><strong>Dirigido por:</strong> {$voluntario}</p>
                <p><strong>Plazas disponibles:</strong> {$this->actividad->getPlazas()}</p>
        EOF;


        //apuntarse para ser el voluntario de la actividad
        if ($this->actividad->getVoluntario() === null) {
            $html .= '<form method="post">';  
            $html .= '<input type="hidden" name="id" value="' . $this->actividad->getId() . '">';
            $html .= '<button type="submit" name="ofrecer">Ofrecerme para dirigir la actividad</button>';
            $html .= '</form>';

        }else{
            $html .= '<p>Un voluntario ya se ha ofrecido para dirigir esta actividad.</p>';
        }
    ...
    {
        if (isset($datos['ofrecer']) && $this->actividad && !$this->actividad->getVoluntario()) {
            $this->actividad->setVoluntario("Juan Pérez"); // Simulación de asignación de voluntario
            $this->actividad->setDirigida(true);
            return ["<p>¡Te has ofrecido a dirigir la actividad con éxito!</p>"];
        }
        return [];
    }
}
